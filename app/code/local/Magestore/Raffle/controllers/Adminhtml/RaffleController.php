...
class Magestore_Raffle_Adminhtml_RaffleController extends Mage_Adminhtml_Controller_Action
{
	protected function _initAction(){
		$this->loadLayout()
			->_setActiveMenu('raffle/raffle')
			->_addBreadcrumb(Mage::helper('adminhtml')->__('Raffles Manager'), Mage::helper('adminhtml')->__('Raffle Manager'));
		return $this;
	}
	...
	public function editAction() {
		...
		if ($model->getId() || $id == 0) {
			...
			$this->loadLayout();
			$this->_setActiveMenu('raffle/raffle');

			$this->_addBreadcrumb(Mage::helper('adminhtml')->__('Raffle Manager'), Mage::helper('adminhtml')->__('Raffle Manager'));
			$this->_addBreadcrumb(Mage::helper('adminhtml')->__('Raffle News'), Mage::helper('adminhtml')->__('Raffle News'));

			$this->getLayout()->getBlock('head')->setCanLoadExtJs(true);
			$this->_addContent($this->getLayout()->createBlock('raffle/adminhtml_raffle_edit'))
				->_addLeft($this->getLayout()->createBlock('raffle/adminhtml_raffle_edit_tabs'));

			$this->renderLayout();
		} else {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('raffle')->__('Raffle does not exist'));
			$this->_redirect('*/*/');
		}
	}
	...
	public function exportCsvAction(){
		$fileName   = 'raffle.csv';
		$content	= $this->getLayout()->createBlock('raffle/adminhtml_raffle_edit_tab_tickets')->getCsv();
		$this->_prepareDownloadResponse($fileName,$content);
	}

	public function exportXmlAction(){
		$fileName   = 'raffle.xml';
		$content	= $this->getLayout()->createBlock('raffle/adminhtml_raffle_grid')->getXml();
		$this->_prepareDownloadResponse($fileName,$content);
	}
}